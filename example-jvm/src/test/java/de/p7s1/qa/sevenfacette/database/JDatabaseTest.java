...
class JDatabaseTest {
  ...
  @BeforeAll
  static void setUp() {
    dbConfig = new DatabaseConfig("jdbc:h2:mem:test;DB_CLOSE_DELAY=-1", "org.h2.Driver", "", "", true, false);

    // Create table
    dbPreparationStatements = new DbStatements();
    dbPreparationStatements.add("create table customer(id integer, name varchar(10))");
    dbClient = DFactory.createDatabase(dbConfig);
    dbClient.executeStatements(dbPreparationStatements);
