 ...
 */
fun <T : Any> Verify<T>.jClass() = prop("class") { it::class.java }
 ...
 * @see [isInstanceOf]
 */
fun <T : Any> Verify<T>.hasClass(jclass: Class<out T>) = given { actual ->
 ...
 * @see [isNotInstanceOf]
 */
fun <T : Any> Verify<T>.doesNotHaveClass(jclass: Class<out T>) = given { actual ->
    if (jclass != actual.javaClass) return
 ...
 * @see [hasClass]
 */
fun <T : Any, S : T> Verify<T>.isInstanceOf(jclass: Class<S>): Verify<S> = transform { actual ->
 ...
 * @see [doesNotHaveClass]
 */
fun <T : Any> Verify<T>.isNotInstanceOf(jclass: Class<out T>) = given { actual ->
 ...
 * ```
 */
fun <T, P> Verify<T>.prop(callable: KCallable<P>) = prop(callable.name) { callable.call(it) }
 ...
 */
fun <T : Any> Verify<T>.isDataClassEqualTo(expected: T) = given { actual ->
...
private fun <T> Verify<T>.isDataClassEqualToImpl(expected: T, kclass: KClass<*>?): Unit = given { actual ->
 ...
 * ```
 */
fun <T : Any> Verify<T>.isEqualToIgnoringGivenProperties(other: T, vararg properties: KProperty1<T, Any?>) {
    all {
