 ...
 */
fun Verify<Map<*, *>>.size() = prop("size", Map<*, *>::size)
 ...
 * @see [isNullOrEmpty]
 */
fun Verify<Map<*, *>>.isEmpty() = given { actual ->
 ...
 * @see [isEmpty]
 */
fun Verify<Map<*, *>>.isNotEmpty() = given { actual ->
 ...
 * @see [isEmpty]
 */
fun Verify<Map<*, *>?>.isNullOrEmpty() = given { actual ->
 ...
 */
fun Verify<Map<*, *>>.hasSize(size: Int) {
    size().isEqualTo(size)
}
 ...
 */
fun Verify<Map<*, *>>.hasSameSizeAs(other: Map<*, *>) = given { actual ->
 ...
 * @see [doesNotContain]
 */
fun <K, V> Verify<Map<K, V>>.contains(key: K, value: V) = given { actual ->
    if (actual[key] == value) {
        return
    }
 ...
 * @see [doesNotContain]
 */
fun <K, V> Verify<Map<K, V>>.contains(element: Pair<K, V>) {
 ...
 * @see [containsExactly]
 */
fun <K, V> Verify<Map<K, V>>.containsAll(vararg elements: Pair<K, V>) = given { actual ->
 ...
 * @see [contains]
 */
fun <K, V> Verify<Map<K, V>>.doesNotContain(key: K, value: V) = given { actual ->
    if (actual[key] != value) {
        return
    }
 ...
 * @see [contains]
 */
fun <K, V> Verify<Map<K, V>>.doesNotContain(element: Pair<K, V>) {
 ...
 * @see [containsAll]
 */
fun <K, V> Verify<Map<K, V>>.containsOnly(vararg elements: Pair<K, V>) = given { actual ->
 ...
 * ```
 */
fun <K, V> Verify<Map<K, V>>.key(key: K): Verify<V> = transform("${name ?: ""}${show(key, "[]")}") { actual ->
    if (key in actual) {
        actual.getValue(key)
    } else {
